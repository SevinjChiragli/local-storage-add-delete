// Удалить товар с DOM и также с localstorage. Можете использовать массив goods
let goods = [
  {
...
const showList = () => {
  ...
  goods.forEach((item) => {
    ...
    li.querySelector("button").addEventListener("click", () => {
      ...
      let chosenItem = jsbag.some((item2) => {
        return item.product_name === item2.product_name;
      });
      if (chosenItem) {
      } else {
        jsbag.push(item);
    ...
    li.querySelector(".btn2").addEventListener("click", () => {
      li.remove();
      let jsbag = JSON.parse(localStorage.getItem("bag"));
      jsbag = jsbag.filter((item2) => {
